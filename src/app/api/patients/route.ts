...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (keyword) {
      keywordCondition = ` AND (p.Xm LIKE '%${keyword}%' OR y.Zdmc LIKE '%${keyword}%' OR y.xbs LIKE '%${keyword}%' OR y.zlh = '${keyword}' OR p.Kh LIKE '%${keyword}%' OR p.Sfz LIKE '%${keyword}%')`;
    }

    // 门诊就诊查询 - 关联 XT_BRJBXXK 患者基本信息表
    const mzQuery = `
      SELECT * FROM (
        SELECT 
